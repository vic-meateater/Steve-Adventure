...
namespace SteveAdventure
{
    public sealed class EnemyBrain : BrainFSM
    {
        ...
        public EnemyBrain(Mover mover, Vector3[] waypoints, EnemyVision enemyVision,
            ...
            Transform enemyTransform, Collider2D collider, AnimationHandler animationHandler, IEnemySounds enemySounds)
        {
            _enemyVision = enemyVision;

            var idleState = new IdleState(waitDuration);
            var patrolState = new PatrolState(mover, waypoints, collider, enemyVision, animatorController, enemySounds);
            var followState = new FollowState(mover, enemyVision, animatorController, enemySounds);
            var attackState = new AttackState(mover, enemyVision, animatorController, damage, attackCooldown,
                animationHandler, enemySounds);
